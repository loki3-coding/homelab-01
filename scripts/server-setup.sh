...
phase8_summary() {
    ...
    log "  2. Reboot the server to apply all changes"
    log "  3. Configure SSH hardening (run setup-ssh.sh)"
    log "  4. Set up DNS resolution (disable systemd-resolved)"
    log "  5. Start services (see scripts/start-all-services.sh)"
    log "  6. Configure Pi-hole DNS"
    log "  7. Set up PostgreSQL and Gitea"
    echo ""
